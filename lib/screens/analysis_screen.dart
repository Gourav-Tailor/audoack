...
class _AnalysisScreenState extends State<AnalysisScreen> {
  ...
  Future<void> loadAnalysis() async {
    ...
    try {
      final result = await widget.apiService.getLatestAnalysis(
      ...
      if (mounted) {
        setState(() => error = 'Unable to load latest analysis.');
      }
    } finally {
      if (mounted) {
        setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            onPressed: refreshing ? null : loadAnalysis,
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      'Automatically fetch latest analysis every $pollingInterval seconds.',
                      style: const TextStyle(color: Colors.grey),
                    ),
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 20),
            if (a.filename != null)
              _analysisRow(Icons.audiotrack, 'File', a.filename!),
            _analysisRow(Icons.info_outline, 'Status', a.status),
            if (a.emotionalTone != null)
              _analysisRow(
                Icons.mood,
                'Emotional Tone',
                a.emotionalTone!,
              ),
            if (a.emotionalIntensity != null)
              _analysisRow(
                Icons.bolt,
                'Emotional Intensity',
                a.emotionalIntensity!,
              ),
            if (a.audioQuality != null)
              _analysisRow(
                Icons.high_quality,
                'Audio Quality',
                a.audioQuality!,
            ...
            _analysisRow(
              ...
              a.backgroundNoisePresent
                  ? (a.backgroundNoiseType?.isNotEmpty == true
                      ? a.backgroundNoiseType!
                      : 'Present')
                  : 'None',
            ),
            if (a.backgroundNoiseSeverity != null)
              _analysisRow(
                Icons.warning_amber,
                'Noise Severity',
                a.backgroundNoiseSeverity!,
              ),
            _analysisRow(
              Icons.people,
              'Speaker Overlap',
              a.speakerOverlapPresent ? 'Present' : 'None',
            ),
            _analysisRow(
              Icons.pause_circle_outline,
              'Long Silence',
              a.longSilencePresent ? 'Present' : 'None',
            ),
